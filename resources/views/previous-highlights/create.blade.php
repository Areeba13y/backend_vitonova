{{-- resources/views/previous-highlights/create.blade.php --}}
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-start gap-3">
            <div class="w-10 h-10 rounded-xl bg-green-50 text-green-600 flex items-center justify-center flex-shrink-0">
                <i class="fas fa-star-plus"></i>
            </div>
            <div>
                <h2 class="text-xl font-semibold text-gray-800">Add New Highlight</h2>
                <p class="text-sm text-gray-500 mt-1">Create a new previous event highlight</p>
            </div>
        </div>
            ...
            <div class="flex flex-col lg:flex-row lg:items-end gap-4">
                ...
                <div class="flex w-full lg:w-auto gap-3">
                    ...
                    <button type="submit" id="submitBtn" class="w-full lg:w-auto px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors text-sm font-medium">
                        <span id="submitText"><i class="fas fa-save mr-2"></i>Save Highlight</span>
...
$(document).ready(function() {
    ...
    $('#highlightForm').submit(function(e) {
        ...
        $.ajax({
            url: '{{ route("previous-highlights.store") }}',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            error: function(xhr) {
                if (xhr.status === 422) {
                    ...
                    Toast.fire({
                        icon: 'error',
                        title: xhr.responseJSON.message || 'Error creating highlight'
                    });
                }
            },
            complete: function() {
