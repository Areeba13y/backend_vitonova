        ...
        <form id="eventForm" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="flex flex-col lg:flex-row lg:items-end gap-4">
                <div class="flex-1">
                    ...
                    <input id="external_link" type="url" name="external_link" value="{{ $event->external_link }}"
                           class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                ...
                <div class="w-full lg:w-64 space-y-4">
                    ...
                    <div>
                        ...
                        <div id="currentImageContainer" class="relative cursor-pointer group rounded-lg overflow-hidden border-2 border-gray-200 hover:border-green-400 transition-all">
                            <img id="currentImageImg" src="{{ asset($event->image) }}" alt="{{ $event->title }}" class="w-full h-40 object-cover">
...
$(document).ready(function() {
    let currentImageSrc = '{{ $event->image ? asset($event->image) : "" }}';
    let newImageSrc = '';
